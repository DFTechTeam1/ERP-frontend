<template>
    <div>
        ...
        <v-row>
            ...
            <v-col
                v-if="detailProject.permission_list.list_member && detailProject.permission_list.list_entertainment_member"
                cols="12"
                md="5">
                <v-skeleton-loader v-if="loading" type="card" height="800"></v-skeleton-loader>
                <master-card
                    v-else
                    :min-height="detailProject.permission_list.list_request_song ? 830 : 400"
                    :max-height="detailProject.permission_list.list_request_song ? 830 : 400"
                    height="100%">
                    <v-toolbar color="surface">
                        <v-toolbar-title class="d-flex align-center justify-space-between">
                            <v-btn-toggle
                                ...
                                <v-btn value="teams" v-if="detailProject.permission_list.list_member">
                                    {{ $t('teams') }}
                                </v-btn>
                                <v-btn value="entertainment" v-if="detailProject.permission_list.list_entertainment_member">
                                    {{ $t('entertainment') }}
                                </v-btn>
                            </v-btn-toggle>
                        </v-toolbar-title>

                        <v-btn icon v-if="detailProject.permission_list.add_team_member">
                            <v-icon
            ...
            <v-col
                cols="12"
                :md="detailProject.permission_list.list_member && detailProject.permission_list.list_entertainment_member ? '7' : '12'">
                <v-row>
                    <v-col cols="12"
                        :md="!detailProject.permission_list.list_member ? '6': '12'">
                    ...
                    <v-col cols="12"
                        :md="!detailProject.permission_list.list_member ? '6': '12'"
                        v-if="detailProject.permission_list.list_request_song">
                        <v-skeleton-loader v-if="loading" type="card" height="400"></v-skeleton-loader>
                        <master-card
                            v-else
                            class="mb-2"
                            ...
                            <v-toolbar color="surface" class="pe-4">
                                ...
                                <div class="d-flex align-center ga-2"
                                    v-if="detailProject.permission_list.create_request_song || detailProject.permission_list.distribute_request_song">
                                    ...
                                    <v-menu>
                                        ...
                                        <v-list>
                                            <v-list-item @click.prevent="showSongForm = true"
                                                v-if="detailProject.permission_list.create_request_song">
                                                <v-list-item-title>
                                                    <v-icon
                                                        :icon="mdiPlus"
                                                        size="15"></v-icon>
                                            ...
                                            <v-list-item @click.prevent="showBulkSongDistribute = true"
                                                v-if="detailProject.permission_list.distribute_request_song">
                                                <v-list-item-title>
                                                    <v-icon
            ...
            <div class="w-100 tab-detail-project">
                <v-tabs show-arrows v-model="tab" :direction="tabDirection">
                    <v-tab
                        :text="t('task')"
                        color="primary"
                        v-if="detailProject.permission_list.list_task"
...
onMounted(() => {
    ...
    canMoveToProgress.value = detailProject.value.permission_list.move_task_to_progress;
    canMoveToReviewClient.value = detailProject.value.permission_list.move_task_to_review_client;
    canMoveToReviewPm.value = detailProject.value.permission_list.move_task_to_review_pm;
    canMoveToRevise.value = detailProject.value.permission_list.move_task_to_revise;
    canMoveToCompleted.value = detailProject.value.permission_list.move_task_to_completed;
    canMoveTask.value = detailProject.value.permission_list.move_task;
    canAddTask.value = detailProject.value.permission_list.add_task;
    canDeleteTask.value = detailProject.value.permission_list.delete_task;
