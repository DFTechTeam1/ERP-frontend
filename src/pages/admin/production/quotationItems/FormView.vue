...
const dialogTitle = ref(t('createQuotationItem'));
...
watch(props, (values) => {
    if (values) {
        ...
        if (Object.keys(values.selectedItem).length > 0) {
            ...
            setFieldValue('name', '');
            dialogTitle.value = t('createQuotationItem');
        }
    }
});
...
const validateData = handleSubmit(async (values) => {
    loading.value = true;
    let method = Object.keys(props.selectedItem).length > 0 ? 'updateQuotationItem' : 'createQuotationItem';
    let id = Object.keys(props.selectedItem).length > 0 ? props.selectedItem.uid : null;
