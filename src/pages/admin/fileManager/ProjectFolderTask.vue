<template>
	<div>
		...
		<template v-else>
			<div class="folders-wrapper" v-if="tasks.length">
				<div class="folders-item"
					v-for="(folder, ff) in tasks"
					:key="ff"
					@click.prevent="detailImageTask(folder)">
					...
					<div class="folders-item__value">
						...
						<p class="desc">{{ folder.image }} {{ $t('images') }}</p>
					</div>
				</div>
			</div>
			<div v-else>
...
function detailImageTask(task) {
	router.push(`/admin/production/files/${route.params.year}/${route.params.project}/images/${task.id}`)
}
